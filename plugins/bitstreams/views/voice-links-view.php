<?php
/**
 * Bitstreams Plugin Voice URL Dashboard View
 */
...
if (function_exists('has_permission')) {
    $canView = has_permission('bitstreams_view') || has_permission('voiceLinks.view') || has_permission('voiceLinks_view');
    $canEdit = has_permission('bitstreams_edit') || has_permission('voiceLinks.edit') || has_permission('voiceLinks_edit');
}

if (!$canView) {
    http_response_code(403);
    die("Unauthorized: You do not have permission to view Voice Links.");
}

$pageRoute = function_exists('url_for') ? url_for('bitstreams_voice_links') : 'index.php?route=bitstreams_voice_links';
...
if (isset($_GET['export'])) {
    ...
    bitstreams_export_voice_links_csv();
}
...
if ($action === "delete" && isset($_GET['id'])) {
    ...
    bitstreams_delete_voice_link($_GET['id']);
...
if ($_SERVER["REQUEST_METHOD"] === "POST" && isset($_POST['import'])) {
    ...
    if (!empty($_FILES['csv_file']['tmp_name'])) {
        bitstreams_import_voice_links_csv($_FILES['csv_file']['tmp_name']);
    }
...
|--------------------------------------------------------------------------
*/
$data = bitstreams_get_voice_links();
...
<div class="container-fluid p-4">
    ...
    <div class="d-flex justify-content-between align-items-center mb-4">
        <div>
            <h2><i class="fa-solid fa-phone-volume text-primary me-2"></i>Voice URL Dashboard</h2>
            <p class="text-muted mb-0">Organize and query voice system endpoints, SIP gateways, and telephony monitoring URLs.</p>
        </div>
        <div>
            <?php if ($canEdit): ?>
    ...
    <div class="card mb-4 border-0 shadow-sm">
        <div class="card-body d-flex justify-content-between align-items-center">
            ...
            <div class="text-muted small fw-bold">
                Categories: <span class="text-primary"><?= count($grouped) ?></span> | Total Voice URLs: <span class="text-info"><?= count($data) ?></span>
            </div>
        </div>
    </div>
    ...
    <div class="row">
        ...
        <div class="col-md-9">
            ...
            <?php if ($canEdit): ?>
                <div class="card mb-4 shadow-sm border-0">
                    <div class="card-header bg-white py-3">
                        <h6 class="mb-0 fw-bold"><i class="fa-solid fa-plus me-2 text-primary"></i>Add New Voice URL</h6>
                    </div>
                    <div class="card-body">
                        <form method="POST" action="<?= $pageRoute ?>" class="row g-2">
                            ...
                            <div class="col-md-3">
                                ...
                                <input class="form-control" name="category" placeholder="e.g. SIP Trunking" required>
                            </div>

                            <div class="col-md-2">
                                <label class="form-label small fw-bold mb-1">Device</label>
                                <input class="form-control" name="device" placeholder="e.g. MetaSwitch 1" required>
                            </div>

                            <div class="col-md-3">
                                <label class="form-label small fw-bold mb-1">Purpose</label>
                                <input class="form-control" name="purpose" placeholder="e.g. CDR Export API" required>
                            </div>

                            <div class="col-md-3">
                                <label class="form-label small fw-bold mb-1">URL</label>
                                <input class="form-control" name="url" placeholder="http://... or sip:..." required>
                            </div>
            ...
            <!-- ACCORDION LIST -->
            <div class="accordion shadow-sm rounded overflow-hidden" id="voiceAccordion">

                <?php if (empty($grouped)): ?>
                    <div class="card p-4 text-center text-muted">
                        <i class="fa-solid fa-phone-slash fs-1 mb-2 text-secondary"></i>
                        <p class="mb-0">No voice URLs added yet. Use the form above to add a new voice URL or import a CSV file.</p>
                    </div>
                <?php else: ?>
                    <?php
                    $i = 0;
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title"><i class="fa-solid fa-file-csv me-2 text-success"></i>Import Voice URLs from CSV</h5>
